# Happy-new-year-wish-sinhala-hindu-new-year-wish 🎉

This is a simple open-source webpage to share **Sinhala & Hindu New Year (Aluth Avurudu)** greetings for **2025**.  
Perfect for sharing a warm wish with friends, family, and your community! 🇱🇰
<!-- ![Logo](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/th5xamgrr6se0x5ro4g6.png) -->

## Screenshots

![Screenshot](assests/beforeCelebrate.png)

![Screenshot](assests/celebrate.png)

## 🌟 What’s Inside?

- Neothemed web page
- A single static HTML file (`index.html`)
- Responsive and colorful design
- Ready-to-use New Year greeting
- no frameworks – just pure HTML , CSS & JavaScript,
- Assets: 📷 Screenshot & 🎞️ Animated preview in assets/
## Authors

- [@Mavros-Lykos](https://github.com/Mavros-Lykos)

## Demo

![Preview of webpage](assests/preview.gif)

## Run Locally

Clone the project

```bash
  git clone https://github.com/Mavros-Lykos/Happy-new-year-wish-sinhala-hindu-new-year-wish.git
```

Go to the project directory and open with live server(extention)


## Feedback

If you have any feedback, please reach out to us at mavroslykosedu@proton.me


## 📜 License

[![License: CC0-1.0](https://licensebuttons.net/p/zero/1.0/88x31.png)](http://creativecommons.org/publicdomain/zero/1.0/)

This project is licensed under the **Creative Commons Zero v1.0 Universal (CC0 1.0)** license.


📄 [View the full license text](http://creativecommons.org/publicdomain/zero/1.0/)  
📁 Or see the [`LICENSE`](LICENSE) file in this repository.


